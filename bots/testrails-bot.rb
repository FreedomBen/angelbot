...
class TestrailsBot < SlackbotFrd::Bot
  ...
  def translate_single_testrails(extracted_testrail, sc, user, channel, _message, thread_ts)
  ...
  def translate_multiple_testrails(extracted_testrails, sc, user, channel, _message, thread_ts)
    change_api = Testrails::Change.new(
      username: $slackbotfrd_conf['testrail_username'],
      password: $slackbotfrd_conf['testrail_password']
    )
  ...
  def extract_testrailcases(str)
    str.scan(/c(\d{5,9})/i).map(&:first).uniq
  end
